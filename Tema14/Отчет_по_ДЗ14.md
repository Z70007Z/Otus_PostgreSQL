...
# Выполняю на сервере 3 команды cоздания пустой таблицы test:
...
# Выполняю команду для проверки:
  ...
  ----+-------     
    1 | test
# И вижу что данные с сервера 1 таблици test реплицируются.
  SELECT * FROM test2 LIMIT 5;
  id |  fio2
  ----+-------     
    1 | test2
# И вижу что данные с сервера 2 таблици test2 реплицируются.
...
# Cкопируем базы с основного сервера на резервный с помощью команды pg_basebackup.
sudo su - postgres
...
# Идем на сервер vm1 и доавляем данные в таблицу test:
INSERT INTO test (fio) VALUES ('testee')
INSERT 0 1
# Идем на сервер vm2 и доавляем данные в таблицу test2:
...
# Пролем нет, если делать не раз и внимательно поработать с файлами конфигурации.
